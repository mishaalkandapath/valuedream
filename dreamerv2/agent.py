...
class WorldModel(common.Module):
  ...
  def video_pred(self, data, key):
    ...
    recon = decoder(code_vec=self.rssm.get_feat(states), actions=data['action'][:6, :5], hor=1)[key].mode()[:6]
    init = {k: v[:, -1] for k, v in states.items()}
    prior = self.rssm.imagine(data['action'][:6, 5:], init)
    openl = decoder(code_vec=self.rssm.get_feat(prior), actions=data['action'][:6, 5:], hor=1)[key].mode()
