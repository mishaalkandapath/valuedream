...
class ShallowAgent(common.Module):
  ...
  def __init__(self, config, obs_space, act_space, step):
    ...
    self.wm = ShallowWorldModel(config, obs_space, self.tfstep)
    self._task_behavior = ShallowActorCritic(config, self.act_space, self.tfstep)
...
class ShallowWorldModel(common.Module):
  ...
  def __init__(self, config, obs_space, tfstep):
    ...
    self.heads = {}
    self.heads['decoder'] = common.RecurrenShallowDecoder(shapes, **config.decoder) 
  ...
  def loss(self, data, state=None):
    ...
    inp = feat
    out, indices = head(inp, data["action"], hor=8)
    dists = out if isinstance(out, dict) else {"decoder": out}
    for key, dist in dists.items():
      like = tf.cast(dist.log_prob(self.multi_step_helper(data, indices)), tf.float32)
      likes[key] = like
...
class ShallowActorCritic(common.Module):
